...
func TestEmpty(t *testing.T) {
	key := []byte("aaa")
	s := NewSkiplist(NewArena(arenaSize))
...
func TestFull(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		err := it.Add(newKey(i), newValue(i), 0)
		if errors.Is(err, ErrArenaFull) {
			foundArenaFull = true
		}
	}

	assert.True(t, foundArenaFull)
...
func TestBasic(t *testing.T) {
	...
	assert.False(t, it.Seek([]byte("key")))

	assert.True(t, it.Seek([]byte("key1")))
	assert.EqualValues(t, "00042", it.Value())
	assert.EqualValues(t, 0, it.Meta())

	assert.True(t, it.Seek([]byte("key2")))
	assert.EqualValues(t, "00052", it.Value())
	assert.EqualValues(t, 100, it.Meta())

	assert.True(t, it.Seek([]byte("key3")))
	assert.EqualValues(t, "00062", it.Value())
	assert.EqualValues(t, 0xffff, it.Meta())

	assert.True(t, it.Seek([]byte("key2")))
	assert.Nil(t, it.Set(val4, 101))
	assert.EqualValues(t, "00072", it.Value())
	assert.EqualValues(t, 101, it.Meta())

	assert.True(t, it.Seek([]byte("key3")))
	assert.Nil(t, it.Delete())
	assert.False(t, it.Valid())
}
...
func TestConcurrentBasic(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		go func(i int) {
			...
			var it Iterator
			it.Init(s)
			assert.True(t, it.Seek(newKey(i)))
			assert.EqualValues(t, newValue(i), it.Value())
		}(i)
	}
	wg.Wait()

	assert.Equal(t, n, length(s))
	assert.Equal(t, n, lengthRev(s))
}
