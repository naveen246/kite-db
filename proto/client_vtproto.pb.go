...
func (m *DeleteRangeRequest) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.EndInclusive) > 0 {
		i -= len(m.EndInclusive)
		copy(dAtA[i:], m.EndInclusive)
		i = encodeVarint(dAtA, i, uint64(len(m.EndInclusive)))
		i--
		dAtA[i] = 0x12
	}
...
func (m *DeleteRangeRequest) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			...
			m.EndInclusive = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skip(dAtA[iNdEx:])
